...
import collections.main.train.PassengerTrain;
import collections.main.train.wagons.PassengerCarriage;
...
public class TrainStation {
  ...
  public static void savePassengerTrainToFile(
      PassengerTrain passengerTrain, String outputStreamFile) {
    try (XMLEncoder xmlEncoder = new XMLEncoder(
        new BufferedOutputStream(new FileOutputStream(outputStreamFile)))) {
  ...
  public static PassengerTrain loadPassengerTrainFromFile(String inputStreamFile) {
    ...
    try (XMLDecoder xmlDecoder = new XMLDecoder(
        new BufferedInputStream(new FileInputStream(inputStreamFile)))) {
  ...
  public static PassengerTrain sortPassengerWagonsByTravelClass(PassengerTrain passengerTrain) {
    passengerTrain.getWagons().sort(Comparator.comparing(PassengerCarriage::getTravelClassIndex)
                .thenComparingInt(PassengerCarriage::getPassengers));
    return passengerTrain;
  }

  public static List<PassengerCarriage> filterPassengerWagonsByTheNumberOfPassengers(
