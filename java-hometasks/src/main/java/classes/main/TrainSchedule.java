...
public class TrainSchedule {
  ...
  public static Train[] createRandom(int length) {
    String[] destinationTemplate = {
      "Lviv", "Brest", "Warsaw", "Adler", "Moscow", "Vilnius", "Kiev"
    };
  ...
  public static Train[] createManually(int length) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      trainArray[i].setEconomyClassSleeper(
          Input.toNumber("Number of economy class sleeper seats - "));
  ...
  public static void findByDestination(Train[] train) {
    ...
    for (Train locomotive : train) {
      if (destination.equalsIgnoreCase(locomotive.destination)) {
        System.out.println(locomotive.toString());
        noResults = false;
      }
    }
    if (noResults) System.out.println("No matches found");
  }
  ...
  public static void findByDestinationHour(Train[] train) {
    ...
    for (Train locomotive : train) {
      if (destination.equalsIgnoreCase(locomotive.destination)
          && (locomotive.getDepartureTime().isAfter(time))) {
        System.out.println(locomotive.toString());
        noResults = false;
      }
    }
  ...
  public static void findByDestinationCompartment(Train[] train) {
    ...
    boolean noResults = true;
    for (Train locomotive : train) {
      if (destination.equalsIgnoreCase(locomotive.destination) && locomotive.getSittingCoaches() > 0) {
        System.out.println(locomotive.toString());
        noResults = false;
      }
    }
