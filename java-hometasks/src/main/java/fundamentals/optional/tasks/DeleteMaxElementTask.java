package fundamentals.optional.tasks;

import fundamentals.optional.PrintDoubleArray;
...
public class DeleteMaxElementTask {
  ...
  public static void run() {
    ...
    System.out.println("\nInitial array:");
    PrintDoubleArray.print2DArray(array);
    int maxInArray = 0;
    for (int i = 0; i < array.length; i++) {
      for (int j = 0; j < array.length; j++) {
        if (array[i][j] > maxInArray) maxInArray = array[i][j];
      }
    }
      ...
      for (int j = 0; j < array.length; j++) {
        if (array[i][j] == maxInArray) {
          for (int k = 0; k < array.length; k++) {
            if (array[i][k] != maxInArray) array[i][k] = marker;
            if (array[k][j] != maxInArray) array[k][j] = marker;
          }
          array[i][j] = marker;
        }
      }
    ...
    for (int i = 0; i < array.length; i++) {
      ...
      for (int j = 0; j < array.length; j++) {
        if (array[i][j] != marker) lineDeleted = false;
        if (array[j][i] != marker) columnDeleted = false;
      }
      if (lineDeleted) linesLeft -= 1;
      if (columnDeleted) columnsLeft -= 1;
    }
    ...
    for (int i = 0; i < array.length; i++) {
      ...
      newArrayColumn = 0;
      if (nextLine) newArrayLine += 1;
      nextLine = false;
    }

    System.out.println("\nRenewed array:");
    PrintDoubleArray.print2DArray(newArray);
  }
}
